...
#             if student_data:
#                 stats['total_students'] = student_data['total']
#                 stats['active_students'] = student_data['active']
            
#             # Get attendance data
#             cursor.execute("""
#                 SELECT 
#                     COUNT(*) as total_records,
#                     SUM(CASE WHEN is_present = TRUE THEN 1 ELSE 0 END) as present_records
#                 FROM attendance 
#                 WHERE course_id = %s
#             """, (course_id,))
#             attendance_data = cursor.fetchone()
#             if attendance_data and attendance_data['total_records'] > 0:
#                 stats['avg_attendance'] = round(
#                     (attendance_data['present_records'] / attendance_data['total_records']) * 100, 2
#                 )
...
#     conn = current_app.get_db_connection()
#     # Initialize stats with default values to prevent errors if queries fail
#     stats = {
#         'total_courses': 0, 'total_students': 0, 'pending_leaves': 0, 'active_batches': 0,
#         'total_topics': 0, 'total_assignments': 0, 'attendance_percentage': 0, 'total_classes': 0,
#         'attended_classes': 0, 'max_leaves': 0, 'remaining_leaves': 0, 'submitted_assignments': 0, 'avg_grade': None
#     }
    
#     if not conn:
#         print("Error: Could not get database connection for stats.")
#         return stats
...
#         if role == 'admin':
#             # Get admin stats in a single, efficient query
#             cursor.execute("""
#                 SELECT 
...
#                 LEFT JOIN students s ON ct.course_id = s.course_id
#                 LEFT JOIN topics t ON ct.course_id = t.course_id AND t.trainer_id = %s
#                 LEFT JOIN assignments a ON t.topic_id = a.topic_id
...
#                     COALESCE(COUNT(att.attendance_id), 0) as total_classes,
#                     COALESCE(SUM(att.is_present), 0) as attended_classes,
#                     c.max_leaves,
#                     (c.max_leaves - (SELECT COUNT(*) FROM leave_applications la WHERE la.student_id = s.student_id AND la.status = 'approved')) as remaining_leaves,
...
#                     (SELECT AVG(grade) FROM assignment_submissions asub WHERE asub.student_id = s.student_id AND asub.grade IS NOT NULL) as avg_grade
#                 FROM students s
...
#                 WHERE s.user_id = %s
#                 GROUP BY s.student_id, c.max_leaves, bs.batch_id
#             """, (user_id,))
...
#             # Final formatting for clean display
#             if 'attendance_percentage' in stats and stats['attendance_percentage'] is not None:
#                 stats['attendance_percentage'] = round(stats['attendance_percentage'], 1)

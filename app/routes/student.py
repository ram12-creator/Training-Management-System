...
#     assignments = []
#     if not conn:
#         return render_template('student/assignments.html', assignments=[])
#     try:
#         cursor = conn.cursor(dictionary=True)
#         cursor.execute("""
#             SELECT
...
#             JOIN assignments a ON bs.batch_id = a.batch_id AND a.is_active = TRUE
#             JOIN topics t ON a.topic_id = t.topic_id
#             LEFT JOIN assignment_submissions asub ON a.assignment_id = asub.assignment_id AND asub.student_id = s.student_id
#             WHERE u.user_id = %s
#             ORDER BY a.due_date DESC;
#         """, (current_user.user_id,))
#         assignments = cursor.fetchall()
...
#         cursor.execute("""
#             INSERT INTO assignment_submissions (assignment_id, student_id, file_path, is_late, submitted_at, evaluation_status) 
#             VALUES (%s, %s, %s, %s, NOW(), %s)
#             ON DUPLICATE KEY UPDATE 
#             file_path = VALUES(file_path), is_late = VALUES(is_late), submitted_at = NOW(),
#             evaluation_status = VALUES(evaluation_status), grade = NULL, auto_grade = NULL, feedback = NULL, auto_feedback = NULL
#         """, (assignment_id, student_id, file_path_relative, is_late, 'pending'))
